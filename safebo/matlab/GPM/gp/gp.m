...
%
% Copyright (c) by Carl Edward Rasmussen and Hannes Nickisch, 2019-03-24.
...
try                                                  % call the inference method
  ...
  msgstr = lasterr;
  if nargin>7,error('MATLAB:posdef','Inference method failed [%s]', msgstr); else 
    warning('Inference method failed [%s] .. attempting to continue',msgstr)
    varargout = {NaN, vec2any(hyp,zeros(numel(any2vec(hyp)),1))};
    rethrow(ME);
    return % go on
  end
end
