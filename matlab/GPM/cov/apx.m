...
use_GPU = 0;
  ...
  if isWneg                  % switch between Cholesky and LU decomposition mode
    ...
    if nargout>4, L = -diag(W)*Q; end                              % overwrite L
      ...
      if use_GPU
        sW = gpuArray(sqrt(W)); if ~isgpuarray(K), K = gpuArray(K); end 
        L = chol((eye(n)+sW*sW'.*K));             % Cholesky factor of B
        ldB2 = gather(sum(log(diag(L))));                                    % log(det(B))/2
        solveKiW = @(r) gather(bsxfun(@times,solve_chol(L,bsxfun(@times,gpuArray(r),sW)),sW));
        if nargout>2, Q = gather(bsxfun(@times,1./sW,solve_chol(L,diag(sW)))); end
        L = gather(L);
      else
